...
make_match() {
    ...
    for match in $MATCHES; do
        ...
        if [ "$vp_red" -gt "$vp_blue" -a "$vp_red" -gt "$vp_green" ]
        then
          frst=$vp_red
          frst_color="red"
        ...
        elif [ "$vp_blue" -gt "$vp_red" -a "$vp_blue" -gt "$vp_green" ]
        then
          frst=$vp_blue
          frst_color="blue"
        ...
        [ $vp_max = $vp_min ] && frst_victory_ratio=0 || frst_victory_ratio=$(( 100 * (frst - vp_min) / (vp_max - vp_min) ))
        ...
        frst_difficulty=$((100 * frst_secure / vp_remaining))
            ...
            echo "<p>"
            for world_id in $(jq ".[$match].all_worlds.${frst_color}[]" matches.json); do
                world_name=$(jq -r ".[]|select(.id==$world_id).name" worlds.json)
                world_pop=$(jq -r ".[]|select(.id==$world_id).population" worlds.json)
                echo "<span class='$frst_color' style='color:$frst_color;' id='$world_id'>$world_id :$world_pop: $world_name</span><br>"
            done
            echo "</p>"
            kills=$(jq ".[$match].kills.$frst_color" matches.json)
            deaths=$(jq ".[$match].deaths.$frst_color" matches.json)
            ...
            echo "<div class='green' style='display:inline-block; border:solid black 2px; background-color: green; height: 10px; width: 100px;'>
...
<div class='maroon' style='background-color: maroon; width: ${maroon}%; height: 100%;'>
        ...
        scnd_difficulty=$((100 * scnd_secure / vp_remaining))
            ...
            echo "<p>"
            for world_id in $(jq ".[$match].all_worlds.${scnd_color}[]" matches.json); do
                world_name=$(jq -r ".[]|select(.id==$world_id).name" worlds.json)
                world_pop=$(jq -r ".[]|select(.id==$world_id).population" worlds.json)
                echo "<span class='$scnd_color' style='color:$scnd_color;' id='$world_id'>$world_id :$world_pop: $world_name</span><br>"
            done
            echo "</p>"
            kills=$(jq ".[$match].kills.$scnd_color" matches.json)
            deaths=$(jq ".[$match].deaths.$scnd_color" matches.json)
            [ $deaths = 0 ] && $deaths=1    # to avoid zero div
            kdr=$(( kills * 100 / deaths ))
            if [ "$kdr" -le 100 ]
            then
...
<div class='maroon' style='background-color: maroon; width: ${maroon}%; height: 100%;'>
        ...
        thrd_difficulty=$((100 * thrd_secure / vp_remaining))
            ...
            echo "<p>"
            for world_id in $(jq ".[$match].all_worlds.${thrd_color}[]" matches.json); do
                world_name=$(jq -r ".[]|select(.id==$world_id).name" worlds.json)
                world_pop=$(jq -r ".[]|select(.id==$world_id).population" worlds.json)
                echo "<span class='$thrd_color' style='color:$thrd_color;' id='$world_id'>$world_id :$world_pop: $world_name</span><br>"
            done
            echo "</p>"
            kills=$(jq ".[$match].kills.$thrd_color" matches.json)
            deaths=$(jq ".[$match].deaths.$thrd_color" matches.json)
            [ $deaths = 0 ] && $deaths=1    # to avoid zero div
            kdr=$(( kills * 100 / deaths ))
            if [ "$kdr" -le 100 ]
            then
